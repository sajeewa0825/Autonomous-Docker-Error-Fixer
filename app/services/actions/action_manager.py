...
def _exec_allowed(command: str, container_name: str) -> str:
    ...
    # Docker SDK restart pattern
    if cmd.startswith("client.containers.get(") and cmd.endswith(").restart()"):
        docker_client.containers.get(container_name).restart()
        return "Container restarted successfully. Watcher will reconnect automatically."

    # Docker SDK kill pattern
    ...
    for pref in allowed_prefixes:
        if cmd.startswith(pref):
            ...
            details = stdout or stderr or "Command finished."
            return f"Executed shell command: {pref}. {details}"
